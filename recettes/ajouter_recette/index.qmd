...
function showUrlError(message) {
  ...
  window.scrollTo(0, 0);
  setTimeout(() => error.classList.add("d-none"), 10000);
}
...
async function handleUrlSubmit(e) {
  ...
  try {
    ...
    const pendingContent = { url, submitted_at: new Date().toISOString(), status: "pending" };
    if (submittedBy && submittedBy.trim()) pendingContent.submitted_by = submittedBy.trim();
...
async function createUrlPullRequest(pendingContent, filename, url) {
  ...
  const yamlContent = jsyaml.dump(pendingContent, { indent: 2, lineWidth: -1, noRefs: true });
  ...
  const branchName = await createBranchWithSuffixes({
    owner, repo, headers, baseSha, baseBranchName: `recipe-url/${Date.now()}`
  });
  ...
  if (!createFileResponse.ok) throw new Error(fileData?.message || "Impossible de créer le fichier pending.");
...
⚠️ Cette PR sera traitée automatiquement par le workflow d'import.`;
  ...
  const prResponse = await fetch(
    `https://api.github.com/repos/${owner}/${repo}/pulls`,
    {
      method: "POST",
      headers,
      body: JSON.stringify({ title: `Import recette: ${url}`, body: prBody, head: branchName, base })
    }
  );
  const prData = await safeJson(prResponse);
  if (!prResponse.ok) throw new Error(prData?.message || "Impossible de créer la pull request.");
