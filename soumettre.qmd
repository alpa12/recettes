...
<div id="form-container">
  <form id="recipe-form">
    ...
    <div class="form-group">
      ...
      <small>Détermine dans quelle section du site la recette apparaîtra</small>
    </div>
    ...
    <div id="categories-container">
      <div class="category-item">
        <input type="text" class="category-text" placeholder="Ex: Plats principaux">
        <button type="button" class="btn-remove-category">Supprimer</button>
      </div>
    </div>
    <button type="button" id="btn-add-category">+ Ajouter une catégorie</button>
        ...
        <div class="steps-list">
          <div class="step-item">
            ...
            <div class="step-ingredients">
              <label>Ingrédients pour cette étape</label>
              <div class="step-ingredients-list">
                <div class="ingredient-item">
                  <input type="text" class="ing-nom" placeholder="Nom de l'ingrédient">
                  <input type="number" class="ing-qte" placeholder="Qté" step="0.01" min="0">
                  <input type="text" class="ing-uni" placeholder="Unité">
                  <button type="button" class="btn-remove-step-ingredient">Supprimer</button>
                </div>
              </div>
              <button type="button" class="btn-add-step-ingredient">+ Ajouter un ingrédient</button>
            </div>
            <div class="step-equipment">
              <label>Équipements pour cette étape</label>
              <div class="step-equipment-list">
                <div class="equipment-item">
                  <input type="text" class="equip-name" placeholder="Ex: Mélangeur">
                  <button type="button" class="btn-remove-equipment">Supprimer</button>
                </div>
              </div>
              <button type="button" class="btn-add-equipment">+ Ajouter un équipement</button>
            </div>
            <button type="button" class="btn-remove-step">Supprimer cette étape</button>
          </div>
        </div>
...
.btn-remove-step-ingredient:hover,
...
document.getElementById('btn-add-category').addEventListener('click', function() {
  ...
  const categoryHTML = `
    <div class="category-item">
      <input type="text" class="category-text" placeholder="Ex: Desserts">
      <button type="button" class="btn-remove-category">Supprimer</button>
    </div>
  `;
...
document.getElementById('preparation-container').addEventListener('click', function(e) {
  ...
  if (e.target.classList.contains('btn-add-step-ingredient')) {
    const stepItem = e.target.closest('.step-item');
    const ingredientsList = stepItem.querySelector('.step-ingredients-list');
  ...
  if (e.target.classList.contains('btn-remove-step-ingredient')) {
    e.target.closest('.ingredient-item').remove();
  }
  
  if (e.target.classList.contains('btn-add-equipment')) {
    const stepItem = e.target.closest('.step-item');
    const equipmentList = stepItem.querySelector('.step-equipment-list');
    const equipmentHTML = `
      <div class="equipment-item">
        <input type="text" class="equip-name" placeholder="Ex: Four">
        <button type="button" class="btn-remove-equipment">Supprimer</button>
      </div>
    `;
...
document.getElementById('recipe-form').addEventListener('submit', async function(e) {
  ...
  try {
    ...
    const formData = {
      ...
      type_recette: document.getElementById('type_recette').value,
    ...
    document.querySelectorAll('.category-item').forEach(item => {
      const text = item.querySelector('.category-text').value.trim();
      if (text) {
    ...
    prepSections.forEach(section => {
      ...
      section.querySelectorAll('.step-item').forEach(item => {
        const stepText = item.querySelector('.step-text').value.trim();
        if (stepText) {
          const stepData = {
            etape: stepText
          };
          ...
          item.querySelectorAll('.step-ingredients-list .ingredient-item').forEach(ingItem => {
            const nom = ingItem.querySelector('.ing-nom').value.trim();
            if (nom) {
              const qte = ingItem.querySelector('.ing-qte').value;
              const uni = ingItem.querySelector('.ing-uni').value.trim();
          ...
          if (stepIngredients.length > 0) {
            stepData.ingredients = stepIngredients;
          }
          
          // Collect equipment for this step
          const stepEquipment = [];
          item.querySelectorAll('.step-equipment-list .equipment-item').forEach(eqItem => {
            const equipName = eqItem.querySelector('.equip-name').value.trim();
            if (equipName) {
              stepEquipment.push(equipName);
            }
          });
          if (stepEquipment.length > 0) {
            stepData.equipements = stepEquipment;
          }
          
          etapes.push(stepData);
        }
      });
      if (etapes.length > 0) {
    ...
    // Create pull request
    await createPullRequest(formData.nom_court, yamlContent, formData.nom, token, username, formData.type_recette);
...
function convertToYAML(data) {
  ...
  if (data.categories && data.categories.length > 0) {
    ...
    data.categories.forEach(category => {
      yaml += `  - ${escapeYamlString(category)}\n`;
    });
  }
  ...
  data.preparation.forEach(section => {
    ...
    section.etapes.forEach(step => {
      yaml += `    - etape: ${escapeYamlString(step.etape)}\n`;
      ...
      if (step.equipements && step.equipements.length > 0) {
        yaml += `      equipements:\n`;
        step.equipements.forEach(equip => {
          yaml += `      - ${escapeYamlString(equip)}\n`;
        });
      }
    });
  });
...
async function createPullRequest(nomCourt, yamlContent, nomRecette, token, username, typeRecette) {
  const owner = 'alpa12';
  const repo = 'recettes';
  const baseBranch = 'dev';
  const newBranch = `recipe/${nomCourt}-${Date.now()}`;
  const filePath = `recettes/${typeRecette}/${nomCourt}.yaml`;
